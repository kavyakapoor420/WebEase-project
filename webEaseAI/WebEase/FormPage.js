...
function addOptionSpeechListeners() {
  // Handle checkboxes and radio buttons
  const inputs = document.querySelectorAll(".form-step input[type='checkbox'], .form-step input[type='radio']");
  inputs.forEach((input) => {
    input.addEventListener("change", (event) => {
      if (event.target.checked) {
        const label = document.querySelector(`label[for='${event.target.id}']`);
        if (label) {
          console.log("Option selected:", event.target.id, "Label text:", label.textContent.trim());
  ...
  const selects = document.querySelectorAll(".form-step select");
  selects.forEach((select) => {
    select.addEventListener("change", (event) => {
      const selectedOption = event.target.options[event.target.selectedIndex];
      if (selectedOption) {
        console.log("Select option chosen:", selectedOption.textContent.trim());
        speakText(selectedOption.textContent.trim());
      }
    });
  });
}

function observeDynamicOptions() {
  const preferencesContent = document.getElementById("preferencesContent");
  if (!preferencesContent) return;
      ...
      if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            const newInputs = node.querySelectorAll("input[type='checkbox'], input[type='radio']");
            newInputs.forEach((input) => {
              input.addEventListener("change", (event) => {
                if (event.target.checked) {
                  const label = document.querySelector(`label[for='${event.target.id}']`);
                  if (label) {
                    console.log("Option selected (dynamic):", event.target.id, "Label text:", label.textContent.trim());
                    speakText(label.textContent.trim());
                  }
                }
              });
            ...
            newSelects.forEach((select) => {
              select.addEventListener("change", (event) => {
                const selectedOption = event.target.options[event.target.selectedIndex];
                if (selectedOption) {
                  console.log("Select option chosen (dynamic):", selectedOption.textContent.trim());
                  speakText(selectedOption.textContent.trim());
                }
              });
            });
  ...
  observer.observe(preferencesContent, { childList: true, subtree: true });
}

// Call the functions after DOM content is loaded
document.addEventListener("DOMContentLoaded", () => {
  addOptionSpeechListeners();
  observeDynamicOptions();
});
