document.addEventListener("DOMContentLoaded", () => {
  ...
  const runAgentVoiceInputBtn = document.getElementById("runAgentVoiceInput");
  ...
  if (!SpeechRecognition) {
    ...
    runAgentVoiceInputBtn.disabled = true;
    return;
  }
  ...
  runAgentVoiceInputBtn.addEventListener("click", () => {
    if (recognizing) {
      recognition.stop();
      return;
    }
  ...
  recognition.addEventListener("start", () => {
    ...
    if (runAgentMode) {
      runAgentVoiceInputBtn.textContent = "Stop Voice Input";
    } else {
  ...
  recognition.addEventListener("end", () => {
    ...
    runAgentVoiceInputBtn.textContent = "🤖 Run Agent with Voice Input";
  });
  ...
  recognition.addEventListener("result", async (event) => {
    ...
    if (runAgentMode) {
      // Send transcript to backend server to run agent.py
      try {
        ...
        if (data.error) {
          voiceStatus.textContent = "Error from agent: " + data.error;
        } else {
          voiceStatus.textContent = "Agent response: " + data.result;
        }
      } catch (error) {
        voiceStatus.textContent = "Failed to run agent: " + error.message;
      }
    } else {
